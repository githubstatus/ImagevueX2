...
class UserController extends ivController
{
		...
		if (isset($_POST['save']) && is_array($_POST['user'])) {
			...
			if (empty($newUser['access'])) {
				...
				if ($result) {
					...
					if($this->_getParam('old_login') && $this->_getParam('old_login')!=$this->_getParam('login')) {
						$this->_deleteUser($this->_getParam('old_login'));
						ivMessenger::add(ivMessenger::NOTICE, "User data succesfully saved, please login again");
						$this->_redirect('index.php');
					}

					ivMessenger::add(ivMessenger::NOTICE, "User data succesfully saved");
	...
	private function _deleteUser($login) {
		...
		return ivPool::get('userManager')->deleteUser($login);
